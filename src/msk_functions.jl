...
function analyzenames(task_:: MSKtask,whichstream_:: Streamtype,nametype_:: Nametype)
  res = disable_sigint() do
    @msk_ccall( "analyzenames",Int32,(Ptr{Void},Int32,Int32,),task_.task,whichstream_.value,nametype_.value)
  end
...
function analyzeproblem(task_:: MSKtask,whichstream_:: Streamtype)
  res = disable_sigint() do
    @msk_ccall( "analyzeproblem",Int32,(Ptr{Void},Int32,),task_.task,whichstream_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function analyzesolution(task_:: MSKtask,whichstream_:: Streamtype,whichsol_:: Soltype)
  res = disable_sigint() do
    @msk_ccall( "analyzesolution",Int32,(Ptr{Void},Int32,Int32,),task_.task,whichstream_.value,whichsol_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
appendbarvars{T1}(task:: MSKtask,dim:: Vector{T1}) = appendbarvars(task,convert(Vector{Int32},dim))
...
appendcone{T2,T3}(task:: MSKtask,ct:: Conetype,conepar:: T2,submem:: Vector{T3}) = appendcone(task,ct,Float64(conepar),convert(Vector{Int32},submem))
function appendcone(task_:: MSKtask,ct_:: Conetype,conepar_:: Float64,submem_:: Vector{Int32})
  nummem_ = minimum([ length(submem_) ])
  res = disable_sigint() do
    @msk_ccall( "appendcone",Int32,(Ptr{Void},Int32,Float64,Int32,Ptr{Int32},),task_.task,ct_.value,conepar_,nummem_,submem_ .- Int32(1))
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
appendconeseq{T2,T3,T4}(task:: MSKtask,ct:: Conetype,conepar:: T2,nummem:: T3,j:: T4) = appendconeseq(task,ct,Float64(conepar),Int32(nummem),Int32(j))
function appendconeseq(task_:: MSKtask,ct_:: Conetype,conepar_:: Float64,nummem_:: Int32,j_:: Int32)
  res = disable_sigint() do
    @msk_ccall( "appendconeseq",Int32,(Ptr{Void},Int32,Float64,Int32,Int32,),task_.task,ct_.value,conepar_,nummem_,j_-1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
appendconesseq{T2,T3,T4}(task:: MSKtask,ct:: Vector{Conetype},conepar:: Vector{T2},nummem:: Vector{T3},j:: T4) = appendconesseq(task,ct,convert(Vector{Float64},conepar),convert(Vector{Int32},nummem),Int32(j))
...
appendsparsesymmat{T1,T2,T3,T4}(task:: MSKtask,dim:: T1,subi:: Vector{T2},subj:: Vector{T3},valij:: Vector{T4}) = appendsparsesymmat(task,Int32(dim),convert(Vector{Int32},subi),convert(Vector{Int32},subj),convert(Vector{Float64},valij))
...
function basiscond(task_:: MSKtask)
  nrmbasis_ = Vector{Float64}(1)
  nrminvbasis_ = Vector{Float64}(1)
  res = disable_sigint() do
...
function bktostr(task_:: MSKtask,bk_:: Boundkey)
  ...
  res = disable_sigint() do
    @msk_ccall( "bktostr",Int32,(Ptr{Void},Int32,Ptr{UInt8},),task_.task,bk_.value,str_)
  end
...
chgbound{T2,T3,T4,T5}(task:: MSKtask,accmode:: Accmode,i:: T2,lower:: T3,finite:: T4,value:: T5) = chgbound(task,accmode,Int32(i),Int32(lower),Int32(finite),Float64(value))
...
function conetypetostr(task_:: MSKtask,ct_:: Conetype)
  ...
  res = disable_sigint() do
    @msk_ccall( "conetypetostr",Int32,(Ptr{Void},Int32,Ptr{UInt8},),task_.task,ct_.value,str_)
  end
  str_str = convert(String,str_)
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
...
function deletesolution(task_:: MSKtask,whichsol_:: Soltype)
  res = disable_sigint() do
    @msk_ccall( "deletesolution",Int32,(Ptr{Void},Int32,),task_.task,whichsol_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
dualsensitivity{T1}(task:: MSKtask,subj:: Vector{T1}) = dualsensitivity(task,convert(Vector{Int32},subj))
...
getaslice{T2,T3}(task:: MSKtask,accmode:: Accmode,first:: T2,last:: T3) = getaslice(task,accmode,Int32(first),Int32(last))
...
getaslicenumnz{T2,T3}(task:: MSKtask,accmode:: Accmode,first:: T2,last:: T3) = getaslicenumnz(task,accmode,Int32(first),Int32(last))
...
getbarsj{T2}(task:: MSKtask,whichsol:: Soltype,j:: T2) = getbarsj(task,whichsol,Int32(j))
function getbarsj(task_:: MSKtask,whichsol_:: Soltype,j_:: Int32)
  __tmp_var_0 = getlenbarvarj(task_,(j_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  barsj_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getbarsj",Int32,(Ptr{Void},Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,j_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getbarvarnameindex(task_:: MSKtask,somename_:: AbstractString)
  asgn_ = Vector{Int32}(1)
  index_ = Vector{Int32}(1)
  res = disable_sigint() do
...
getbarxj{T2}(task:: MSKtask,whichsol:: Soltype,j:: T2) = getbarxj(task,whichsol,Int32(j))
function getbarxj(task_:: MSKtask,whichsol_:: Soltype,j_:: Int32)
  __tmp_var_0 = getlenbarvarj(task_,(j_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  barxj_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getbarxj",Int32,(Ptr{Void},Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,j_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getbound{T2}(task:: MSKtask,accmode:: Accmode,i:: T2) = getbound(task,accmode,Int32(i))
...
getboundslice{T2,T3}(task:: MSKtask,accmode:: Accmode,first:: T2,last:: T3) = getboundslice(task,accmode,Int32(first),Int32(last))
...
function getcone(task_:: MSKtask,k_:: Int32)
  conepar_ = Vector{Float64}(1)
  ct_ = Vector{Int32}(1)
  nummem_ = Vector{Int32}(1)
...
function getdouinf(task_:: MSKtask,whichdinf_:: Dinfitem)
  dvalue_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getdouinf",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichdinf_.value,dvalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getdouparam(task_:: MSKtask,param_:: Dparam)
  parvalue_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getdouparam",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,param_.value,parvalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getdualobj(task_:: MSKtask,whichsol_:: Soltype)
  dualobj_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getdualobj",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,dualobj_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getdualsolutionnorms(task_:: MSKtask,whichsol_:: Soltype)
  nrmbars_ = Vector{Float64}(1)
  nrmslc_ = Vector{Float64}(1)
  nrmslx_ = Vector{Float64}(1)
  nrmsnx_ = Vector{Float64}(1)
  nrmsuc_ = Vector{Float64}(1)
  nrmsux_ = Vector{Float64}(1)
  nrmy_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getdualsolutionnorms",Int32,(Ptr{Void},Int32,Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},),task_.task,whichsol_.value,nrmy_,nrmslc_,nrmsuc_,nrmslx_,nrmsux_,nrmsnx_,nrmbars_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getdviolbarvar{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getdviolbarvar(task,whichsol,convert(Vector{Int32},sub))
function getdviolbarvar(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getdviolbarvar",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getdviolcon{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getdviolcon(task,whichsol,convert(Vector{Int32},sub))
function getdviolcon(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getdviolcon",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getdviolcones{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getdviolcones(task,whichsol,convert(Vector{Int32},sub))
function getdviolcones(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getdviolcones",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getdviolvar{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getdviolvar(task,whichsol,convert(Vector{Int32},sub))
function getdviolvar(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getdviolvar",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getinfeasiblesubproblem(task_:: MSKtask,whichsol_:: Soltype)
  inftask_ = Vector{Ptr{Void}}(1)
  res = disable_sigint() do
    @msk_ccall( "getinfeasiblesubproblem",Int32,(Ptr{Void},Int32,Ptr{Ptr{Void}},),task_.task,whichsol_.value,inftask_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getinfname{T2}(task:: MSKtask,inftype:: Inftype,whichinf:: T2) = getinfname(task,inftype,Int32(whichinf))
function getinfname(task_:: MSKtask,inftype_:: Inftype,whichinf_:: Int32)
  infname_ = zeros(UInt8,MSK_MAX_STR_LEN+1)
  res = disable_sigint() do
    @msk_ccall( "getinfname",Int32,(Ptr{Void},Int32,Int32,Ptr{UInt8},),task_.task,inftype_.value,whichinf_,infname_)
  end
...
function getintinf(task_:: MSKtask,whichiinf_:: Iinfitem)
  ivalue_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getintinf",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichiinf_.value,ivalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getintparam(task_:: MSKtask,param_:: Iparam)
  parvalue_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getintparam",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,param_.value,parvalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getlintinf(task_:: MSKtask,whichliinf_:: Liinfitem)
  ivalue_ = Vector{Int64}(1)
  res = disable_sigint() do
    @msk_ccall( "getlintinf",Int32,(Ptr{Void},Int32,Ptr{Int64},),task_.task,whichliinf_.value,ivalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getmemusage(task_:: MSKtask)
  maxmemuse_ = Vector{Int64}(1)
  meminuse_ = Vector{Int64}(1)
  res = disable_sigint() do
...
function getnumparam(task_:: MSKtask,partype_:: Parametertype)
  numparam_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getnumparam",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,partype_.value,numparam_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getparamname{T2}(task:: MSKtask,partype:: Parametertype,param:: T2) = getparamname(task,partype,Int32(param))
function getparamname(task_:: MSKtask,partype_:: Parametertype,param_:: Int32)
  parname_ = zeros(UInt8,MSK_MAX_STR_LEN+1)
  res = disable_sigint() do
    @msk_ccall( "getparamname",Int32,(Ptr{Void},Int32,Int32,Ptr{UInt8},),task_.task,partype_.value,param_,parname_)
  end
...
function getprimalobj(task_:: MSKtask,whichsol_:: Soltype)
  primalobj_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getprimalobj",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,primalobj_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getprimalsolutionnorms(task_:: MSKtask,whichsol_:: Soltype)
  nrmbarx_ = Vector{Float64}(1)
  nrmxc_ = Vector{Float64}(1)
  nrmxx_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getprimalsolutionnorms",Int32,(Ptr{Void},Int32,Ptr{Float64},Ptr{Float64},Ptr{Float64},),task_.task,whichsol_.value,nrmxc_,nrmxx_,nrmbarx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getprosta(task_:: MSKtask,whichsol_:: Soltype)
  prosta_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getprosta",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,prosta_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getpviolbarvar{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getpviolbarvar(task,whichsol,convert(Vector{Int32},sub))
function getpviolbarvar(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getpviolbarvar",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getpviolcon{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getpviolcon(task,whichsol,convert(Vector{Int32},sub))
function getpviolcon(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getpviolcon",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getpviolcones{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getpviolcones(task,whichsol,convert(Vector{Int32},sub))
function getpviolcones(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getpviolcones",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getpviolvar{T2}(task:: MSKtask,whichsol:: Soltype,sub:: Vector{T2}) = getpviolvar(task,whichsol,convert(Vector{Int32},sub))
function getpviolvar(task_:: MSKtask,whichsol_:: Soltype,sub_:: Vector{Int32})
  ...
  viol_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getpviolvar",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{Float64},),task_.task,whichsol_.value,num_,sub_ .- Int32(1),__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getreducedcosts{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getreducedcosts(task,whichsol,Int32(first),Int32(last))
function getreducedcosts(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  redcosts_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getreducedcosts",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getskc(task_:: MSKtask,whichsol_:: Soltype)
  ...
  res = disable_sigint() do
    @msk_ccall( "getskc",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,skc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getskcslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getskcslice(task,whichsol,Int32(first),Int32(last))
function getskcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  skc_ = Vector{Int32}(__tmp_var_0)
  res = disable_sigint() do
    @msk_ccall( "getskcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Int32},),task_.task,whichsol_.value,first_-1,last_-1,skc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getskx(task_:: MSKtask,whichsol_:: Soltype)
  ...
  res = disable_sigint() do
    @msk_ccall( "getskx",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,skx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getskxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getskxslice(task,whichsol,Int32(first),Int32(last))
function getskxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  skx_ = Vector{Int32}(__tmp_var_0)
  res = disable_sigint() do
    @msk_ccall( "getskxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Int32},),task_.task,whichsol_.value,first_-1,last_-1,skx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getslc(task_:: MSKtask,whichsol_:: Soltype)
  ...
  slc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getslc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getslcslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getslcslice(task,whichsol,Int32(first),Int32(last))
function getslcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  slc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getslcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getslx(task_:: MSKtask,whichsol_:: Soltype)
  ...
  slx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getslx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getslxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getslxslice(task,whichsol,Int32(first),Int32(last))
function getslxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  slx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getslxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getsnx(task_:: MSKtask,whichsol_:: Soltype)
  ...
  snx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsnx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getsnxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getsnxslice(task,whichsol,Int32(first),Int32(last))
function getsnxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  snx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsnxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getsolsta(task_:: MSKtask,whichsol_:: Soltype)
  solsta_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getsolsta",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,solsta_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getsolution(task_:: MSKtask,whichsol_:: Soltype)
  ...
  y_ = __tmp_var_8
  res = disable_sigint() do
    @msk_ccall( "getsolution",Int32,(Ptr{Void},Int32,Ptr{Int32},Ptr{Int32},Ptr{Int32},Ptr{Int32},Ptr{Int32},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},),task_.task,whichsol_.value,prosta_,solsta_,skc_,skx_,skn_,__tmp_var_4,__tmp_var_6,__tmp_var_8,__tmp_var_10,__tmp_var_12,__tmp_var_14,__tmp_var_16,__tmp_var_18)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getsolutioni{T2}(task:: MSKtask,accmode:: Accmode,i:: T2,whichsol:: Soltype) = getsolutioni(task,accmode,Int32(i),whichsol)
...
function getsolutioninfo(task_:: MSKtask,whichsol_:: Soltype)
  dobj_ = Vector{Float64}(1)
  dviolbarvar_ = Vector{Float64}(1)
  dviolcon_ = Vector{Float64}(1)
  dviolcone_ = Vector{Float64}(1)
  dviolvar_ = Vector{Float64}(1)
  pobj_ = Vector{Float64}(1)
  pviolbarvar_ = Vector{Float64}(1)
  pviolcon_ = Vector{Float64}(1)
  pviolcone_ = Vector{Float64}(1)
  pviolitg_ = Vector{Float64}(1)
  pviolvar_ = Vector{Float64}(1)
  res = disable_sigint() do
    @msk_ccall( "getsolutioninfo",Int32,(Ptr{Void},Int32,Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},),task_.task,whichsol_.value,pobj_,pviolcon_,pviolvar_,pviolbarvar_,pviolcone_,pviolitg_,dobj_,dviolcon_,dviolvar_,dviolbarvar_,dviolcone_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getsolutionslice{T3,T4}(task:: MSKtask,whichsol:: Soltype,solitem:: Solitem,first:: T3,last:: T4) = getsolutionslice(task,whichsol,solitem,Int32(first),Int32(last))
function getsolutionslice(task_:: MSKtask,whichsol_:: Soltype,solitem_:: Solitem,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  values_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsolutionslice",Int32,(Ptr{Void},Int32,Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,solitem_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getstrparam(task_:: MSKtask,param_:: Sparam)
  ...
  res = disable_sigint() do
    @msk_ccall( "getstrparam",Int32,(Ptr{Void},Int32,Int32,Ptr{Int32},Ptr{UInt8},),task_.task,param_.value,maxlen_,len_,parvalue_)
  end
  parvalue_str = convert(String,parvalue_)
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
...
function getstrparamlen(task_:: MSKtask,param_:: Sparam)
  len_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "getstrparamlen",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,param_.value,len_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getsuc(task_:: MSKtask,whichsol_:: Soltype)
  ...
  suc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsuc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getsucslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getsucslice(task,whichsol,Int32(first),Int32(last))
function getsucslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  suc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsucslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getsux(task_:: MSKtask,whichsol_:: Soltype)
  ...
  sux_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsux",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getsuxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getsuxslice(task,whichsol,Int32(first),Int32(last))
function getsuxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  sux_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getsuxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getvartypelist{T1}(task:: MSKtask,subj:: Vector{T1}) = getvartypelist(task,convert(Vector{Int32},subj))
...
function getxc(task_:: MSKtask,whichsol_:: Soltype)
  ...
  xc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getxc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getxcslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getxcslice(task,whichsol,Int32(first),Int32(last))
function getxcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  xc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getxcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function getxx(task_:: MSKtask,whichsol_:: Soltype)
  ...
  xx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getxx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getxxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getxxslice(task,whichsol,Int32(first),Int32(last))
function getxxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  xx_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getxxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function gety(task_:: MSKtask,whichsol_:: Soltype)
  ...
  y_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "gety",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
getyslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3) = getyslice(task,whichsol,Int32(first),Int32(last))
function getyslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32)
  __tmp_var_0 = ((last_) - (first_))
  __tmp_var_1 = zeros(Float64,__tmp_var_0)
  y_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "getyslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
inputdata{T1,T2,T3,T4,T5,T6,T7,T8,T10,T11,T13,T14}(task:: MSKtask,maxnumcon:: T1,maxnumvar:: T2,c:: Vector{T3},cfix:: T4,aptrb:: Vector{T5},aptre:: Vector{T6},asub:: Vector{T7},aval:: Vector{T8},bkc:: Vector{Boundkey},blc:: Vector{T10},buc:: Vector{T11},bkx:: Vector{Boundkey},blx:: Vector{T13},bux:: Vector{T14}) = inputdata(task,Int32(maxnumcon),Int32(maxnumvar),convert(Vector{Float64},c),Float64(cfix),convert(Vector{Int64},aptrb),convert(Vector{Int64},aptre),convert(Vector{Int32},asub),convert(Vector{Float64},aval),bkc,convert(Vector{Float64},blc),convert(Vector{Float64},buc),bkx,convert(Vector{Float64},blx),convert(Vector{Float64},bux))
function inputdata{T1,T2,T3,T4,T10,T11,T13,T14}(task:: MSKtask,maxnumcon:: T1,maxnumvar:: T2,c:: Vector{T3},cfix:: T4,A:: SparseMatrixCSC{Float64},bkc:: Vector{Boundkey},blc:: Vector{T10},buc:: Vector{T11},bkx:: Vector{Boundkey},blx:: Vector{T13},bux:: Vector{T14})
...
linkfiletostream{T3}(task:: MSKtask,whichstream:: Streamtype,filename:: AbstractString,append:: T3) = linkfiletostream(task,whichstream,filename,Int32(append))
function linkfiletostream(task_:: MSKtask,whichstream_:: Streamtype,filename_:: AbstractString,append_:: Int32)
  res = disable_sigint() do
    @msk_ccall( "linkfiletotaskstream",Int32,(Ptr{Void},Int32,Ptr{UInt8},Int32,),task_.task,whichstream_.value,string(filename_),append_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function onesolutionsummary(task_:: MSKtask,whichstream_:: Streamtype,whichsol_:: Soltype)
  res = disable_sigint() do
    @msk_ccall( "onesolutionsummary",Int32,(Ptr{Void},Int32,Int32,),task_.task,whichstream_.value,whichsol_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function optimizersummary(task_:: MSKtask,whichstream_:: Streamtype)
  res = disable_sigint() do
    @msk_ccall( "optimizersummary",Int32,(Ptr{Void},Int32,),task_.task,whichstream_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
primalrepair{T1,T2,T3,T4}(task:: MSKtask,wlc:: Vector{T1},wuc:: Vector{T2},wlx:: Vector{T3},wux:: Vector{T4}) = primalrepair(task,convert(Vector{Float64},wlc),convert(Vector{Float64},wuc),convert(Vector{Float64},wlx),convert(Vector{Float64},wux))
...
primalsensitivity{T1,T3}(task:: MSKtask,subi:: Vector{T1},marki:: Vector{Mark},subj:: Vector{T3},markj:: Vector{Mark}) = primalsensitivity(task,convert(Vector{Int32},subi),marki,convert(Vector{Int32},subj),markj)
...
putacol{T1,T2,T3}(task:: MSKtask,j:: T1,subj:: Vector{T2},valj:: Vector{T3}) = putacol(task,Int32(j),convert(Vector{Int32},subj),convert(Vector{Float64},valj))
...
putacollist{T1,T2,T3,T4,T5}(task:: MSKtask,sub:: Vector{T1},ptrb:: Vector{T2},ptre:: Vector{T3},asub:: Vector{T4},aval:: Vector{T5}) = putacollist(task,convert(Vector{Int32},sub),convert(Vector{Int64},ptrb),convert(Vector{Int64},ptre),convert(Vector{Int32},asub),convert(Vector{Float64},aval))
function putacollist{T1}(task:: MSKtask,sub:: Vector{T1},A:: SparseMatrixCSC{Float64})
...
putacolslice{T1,T2,T3,T4,T5,T6}(task:: MSKtask,first:: T1,last:: T2,ptrb:: Vector{T3},ptre:: Vector{T4},asub:: Vector{T5},aval:: Vector{T6}) = putacolslice(task,Int32(first),Int32(last),convert(Vector{Int64},ptrb),convert(Vector{Int64},ptre),convert(Vector{Int32},asub),convert(Vector{Float64},aval))
...
putaijlist{T1,T2,T3}(task:: MSKtask,subi:: Vector{T1},subj:: Vector{T2},valij:: Vector{T3}) = putaijlist(task,convert(Vector{Int32},subi),convert(Vector{Int32},subj),convert(Vector{Float64},valij))
...
putarow{T1,T2,T3}(task:: MSKtask,i:: T1,subi:: Vector{T2},vali:: Vector{T3}) = putarow(task,Int32(i),convert(Vector{Int32},subi),convert(Vector{Float64},vali))
...
putarowlist{T1,T2,T3,T4,T5}(task:: MSKtask,sub:: Vector{T1},ptrb:: Vector{T2},ptre:: Vector{T3},asub:: Vector{T4},aval:: Vector{T5}) = putarowlist(task,convert(Vector{Int32},sub),convert(Vector{Int64},ptrb),convert(Vector{Int64},ptre),convert(Vector{Int32},asub),convert(Vector{Float64},aval))
function putarowlist{T1}(task:: MSKtask,sub:: Vector{T1},At:: SparseMatrixCSC{Float64})
...
putarowslice{T1,T2,T3,T4,T5,T6}(task:: MSKtask,first:: T1,last:: T2,ptrb:: Vector{T3},ptre:: Vector{T4},asub:: Vector{T5},aval:: Vector{T6}) = putarowslice(task,Int32(first),Int32(last),convert(Vector{Int64},ptrb),convert(Vector{Int64},ptre),convert(Vector{Int32},asub),convert(Vector{Float64},aval))
...
putbarablocktriplet{T1,T2,T3,T4,T5,T6}(task:: MSKtask,num:: T1,subi:: Vector{T2},subj:: Vector{T3},subk:: Vector{T4},subl:: Vector{T5},valijkl:: Vector{T6}) = putbarablocktriplet(task,Int64(num),convert(Vector{Int32},subi),convert(Vector{Int32},subj),convert(Vector{Int32},subk),convert(Vector{Int32},subl),convert(Vector{Float64},valijkl))
...
putbaraij{T1,T2,T3,T4}(task:: MSKtask,i:: T1,j:: T2,sub:: Vector{T3},weights:: Vector{T4}) = putbaraij(task,Int32(i),Int32(j),convert(Vector{Int64},sub),convert(Vector{Float64},weights))
...
putbarcblocktriplet{T1,T2,T3,T4,T5}(task:: MSKtask,num:: T1,subj:: Vector{T2},subk:: Vector{T3},subl:: Vector{T4},valjkl:: Vector{T5}) = putbarcblocktriplet(task,Int64(num),convert(Vector{Int32},subj),convert(Vector{Int32},subk),convert(Vector{Int32},subl),convert(Vector{Float64},valjkl))
...
putbarcj{T1,T2,T3}(task:: MSKtask,j:: T1,sub:: Vector{T2},weights:: Vector{T3}) = putbarcj(task,Int32(j),convert(Vector{Int64},sub),convert(Vector{Float64},weights))
...
putbarsj{T2,T3}(task:: MSKtask,whichsol:: Soltype,j:: T2,barsj:: Vector{T3}) = putbarsj(task,whichsol,Int32(j),convert(Vector{Float64},barsj))
function putbarsj(task_:: MSKtask,whichsol_:: Soltype,j_:: Int32,barsj_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putbarsj",Int32,(Ptr{Void},Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,j_-1,barsj_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putbarxj{T2,T3}(task:: MSKtask,whichsol:: Soltype,j:: T2,barxj:: Vector{T3}) = putbarxj(task,whichsol,Int32(j),convert(Vector{Float64},barxj))
function putbarxj(task_:: MSKtask,whichsol_:: Soltype,j_:: Int32,barxj_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putbarxj",Int32,(Ptr{Void},Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,j_-1,barxj_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putclist{T1,T2}(task:: MSKtask,subj:: Vector{T1},val:: Vector{T2}) = putclist(task,convert(Vector{Int32},subj),convert(Vector{Float64},val))
...
putconbound{T1,T3,T4}(task:: MSKtask,i:: T1,bk:: Boundkey,bl:: T3,bu:: T4) = putconbound(task,Int32(i),bk,Float64(bl),Float64(bu))
function putconbound(task_:: MSKtask,i_:: Int32,bk_:: Boundkey,bl_:: Float64,bu_:: Float64)
  res = disable_sigint() do
    @msk_ccall( "putconbound",Int32,(Ptr{Void},Int32,Int32,Float64,Float64,),task_.task,i_-1,bk_.value,bl_,bu_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putconboundlist{T1,T3,T4}(task:: MSKtask,sub:: Vector{T1},bk:: Vector{Boundkey},bl:: Vector{T3},bu:: Vector{T4}) = putconboundlist(task,convert(Vector{Int32},sub),bk,convert(Vector{Float64},bl),convert(Vector{Float64},bu))
...
putconboundslice{T1,T2,T4,T5}(task:: MSKtask,first:: T1,last:: T2,bk:: Vector{Boundkey},bl:: Vector{T4},bu:: Vector{T5}) = putconboundslice(task,Int32(first),Int32(last),bk,convert(Vector{Float64},bl),convert(Vector{Float64},bu))
...
putcone{T1,T3,T4}(task:: MSKtask,k:: T1,ct:: Conetype,conepar:: T3,submem:: Vector{T4}) = putcone(task,Int32(k),ct,Float64(conepar),convert(Vector{Int32},submem))
function putcone(task_:: MSKtask,k_:: Int32,ct_:: Conetype,conepar_:: Float64,submem_:: Vector{Int32})
  nummem_ = minimum([ length(submem_) ])
  res = disable_sigint() do
    @msk_ccall( "putcone",Int32,(Ptr{Void},Int32,Int32,Float64,Int32,Ptr{Int32},),task_.task,k_-1,ct_.value,conepar_,nummem_,submem_ .- Int32(1))
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putcslice{T1,T2,T3}(task:: MSKtask,first:: T1,last:: T2,slice:: Vector{T3}) = putcslice(task,Int32(first),Int32(last),convert(Vector{Float64},slice))
...
putdouparam{T2}(task:: MSKtask,param:: Dparam,parvalue:: T2) = putdouparam(task,param,Float64(parvalue))
function putdouparam(task_:: MSKtask,param_:: Dparam,parvalue_:: Float64)
  res = disable_sigint() do
    @msk_ccall( "putdouparam",Int32,(Ptr{Void},Int32,Float64,),task_.task,param_.value,parvalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putintparam{T2}(task:: MSKtask,param:: Iparam,parvalue:: T2) = putintparam(task,param,Int32(parvalue))
function putintparam(task_:: MSKtask,param_:: Iparam,parvalue_:: Int32)
  res = disable_sigint() do
    @msk_ccall( "putintparam",Int32,(Ptr{Void},Int32,Int32,),task_.task,param_.value,parvalue_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function putobjsense(task_:: MSKtask,sense_:: Objsense)
  res = disable_sigint() do
    @msk_ccall( "putobjsense",Int32,(Ptr{Void},Int32,),task_.task,sense_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putqcon{T1,T2,T3,T4}(task:: MSKtask,qcsubk:: Vector{T1},qcsubi:: Vector{T2},qcsubj:: Vector{T3},qcval:: Vector{T4}) = putqcon(task,convert(Vector{Int32},qcsubk),convert(Vector{Int32},qcsubi),convert(Vector{Int32},qcsubj),convert(Vector{Float64},qcval))
...
putqconk{T1,T2,T3,T4}(task:: MSKtask,k:: T1,qcsubi:: Vector{T2},qcsubj:: Vector{T3},qcval:: Vector{T4}) = putqconk(task,Int32(k),convert(Vector{Int32},qcsubi),convert(Vector{Int32},qcsubj),convert(Vector{Float64},qcval))
...
putqobj{T1,T2,T3}(task:: MSKtask,qosubi:: Vector{T1},qosubj:: Vector{T2},qoval:: Vector{T3}) = putqobj(task,convert(Vector{Int32},qosubi),convert(Vector{Int32},qosubj),convert(Vector{Float64},qoval))
...
function putskc(task_:: MSKtask,whichsol_:: Soltype,skc_:: Vector{Stakey})
  ...
  res = disable_sigint() do
    @msk_ccall( "putskc",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,skc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putskcslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,skc:: Vector{Stakey}) = putskcslice(task,whichsol,Int32(first),Int32(last),skc)
function putskcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,skc_:: Vector{Stakey})
  ...
  skc_i32 = Int32[item.value for item in skc_]
  res = disable_sigint() do
    @msk_ccall( "putskcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Int32},),task_.task,whichsol_.value,first_-1,last_-1,skc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function putskx(task_:: MSKtask,whichsol_:: Soltype,skx_:: Vector{Stakey})
  ...
  res = disable_sigint() do
    @msk_ccall( "putskx",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,skx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putskxslice{T2,T3}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,skx:: Vector{Stakey}) = putskxslice(task,whichsol,Int32(first),Int32(last),skx)
function putskxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,skx_:: Vector{Stakey})
  ...
  skx_i32 = Int32[item.value for item in skx_]
  res = disable_sigint() do
    @msk_ccall( "putskxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Int32},),task_.task,whichsol_.value,first_-1,last_-1,skx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putslc end
putslc{T2}(task:: MSKtask,whichsol:: Soltype,slc:: Vector{T2}) = putslc(task,whichsol,convert(Vector{Float64},slc))
function putslc(task_:: MSKtask,whichsol_:: Soltype,slc_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putslc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,slc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putslcslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,slc:: Vector{T4}) = putslcslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},slc))
function putslcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,slc_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putslcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,slc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putslx end
putslx{T2}(task:: MSKtask,whichsol:: Soltype,slx:: Vector{T2}) = putslx(task,whichsol,convert(Vector{Float64},slx))
function putslx(task_:: MSKtask,whichsol_:: Soltype,slx_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putslx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,slx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putslxslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,slx:: Vector{T4}) = putslxslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},slx))
function putslxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,slx_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putslxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,slx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putsnx end
putsnx{T2}(task:: MSKtask,whichsol:: Soltype,sux:: Vector{T2}) = putsnx(task,whichsol,convert(Vector{Float64},sux))
function putsnx(task_:: MSKtask,whichsol_:: Soltype,sux_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsnx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,sux_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putsnxslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,snx:: Vector{T4}) = putsnxslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},snx))
function putsnxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,snx_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsnxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,snx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function putsolution(task_:: MSKtask,whichsol_:: Soltype,skc_:: Vector{Stakey},skx_:: Vector{Stakey},skn_:: Vector{Stakey},xc_:: Vector{Float64},xx_:: Vector{Float64},y_:: Vector{Float64},slc_:: Vector{Float64},suc_:: Vector{Float64},slx_:: Vector{Float64},sux_:: Vector{Float64},snx_:: Vector{Float64})
  ...
  skx_i32 = Int32[item.value for item in skx_]
  res = disable_sigint() do
    @msk_ccall( "putsolution",Int32,(Ptr{Void},Int32,Ptr{Int32},Ptr{Int32},Ptr{Int32},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},Ptr{Float64},),task_.task,whichsol_.value,skc_,skx_,skn_,xc_,xx_,y_,slc_,suc_,slx_,sux_,snx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putsolutioni{T2,T5,T6,T7,T8}(task:: MSKtask,accmode:: Accmode,i:: T2,whichsol:: Soltype,sk:: Stakey,x:: T5,sl:: T6,su:: T7,sn:: T8) = putsolutioni(task,accmode,Int32(i),whichsol,sk,Float64(x),Float64(sl),Float64(su),Float64(sn))
...
putsolutionyi{T1,T3}(task:: MSKtask,i:: T1,whichsol:: Soltype,y:: T3) = putsolutionyi(task,Int32(i),whichsol,Float64(y))
function putsolutionyi(task_:: MSKtask,i_:: Int32,whichsol_:: Soltype,y_:: Float64)
  res = disable_sigint() do
    @msk_ccall( "putsolutionyi",Int32,(Ptr{Void},Int32,Int32,Float64,),task_.task,i_-1,whichsol_.value,y_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function putstrparam(task_:: MSKtask,param_:: Sparam,parvalue_:: AbstractString)
  res = disable_sigint() do
    @msk_ccall( "putstrparam",Int32,(Ptr{Void},Int32,Ptr{UInt8},),task_.task,param_.value,string(parvalue_))
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putsuc end
putsuc{T2}(task:: MSKtask,whichsol:: Soltype,suc:: Vector{T2}) = putsuc(task,whichsol,convert(Vector{Float64},suc))
function putsuc(task_:: MSKtask,whichsol_:: Soltype,suc_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsuc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,suc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putsucslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,suc:: Vector{T4}) = putsucslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},suc))
function putsucslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,suc_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsucslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,suc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putsux end
putsux{T2}(task:: MSKtask,whichsol:: Soltype,sux:: Vector{T2}) = putsux(task,whichsol,convert(Vector{Float64},sux))
function putsux(task_:: MSKtask,whichsol_:: Soltype,sux_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsux",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,sux_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putsuxslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,sux:: Vector{T4}) = putsuxslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},sux))
function putsuxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,sux_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putsuxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,sux_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putvarbound{T1,T3,T4}(task:: MSKtask,j:: T1,bk:: Boundkey,bl:: T3,bu:: T4) = putvarbound(task,Int32(j),bk,Float64(bl),Float64(bu))
function putvarbound(task_:: MSKtask,j_:: Int32,bk_:: Boundkey,bl_:: Float64,bu_:: Float64)
  res = disable_sigint() do
    @msk_ccall( "putvarbound",Int32,(Ptr{Void},Int32,Int32,Float64,Float64,),task_.task,j_-1,bk_.value,bl_,bu_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putvarboundlist{T1,T3,T4}(task:: MSKtask,sub:: Vector{T1},bkx:: Vector{Boundkey},blx:: Vector{T3},bux:: Vector{T4}) = putvarboundlist(task,convert(Vector{Int32},sub),bkx,convert(Vector{Float64},blx),convert(Vector{Float64},bux))
...
putvarboundslice{T1,T2,T4,T5}(task:: MSKtask,first:: T1,last:: T2,bk:: Vector{Boundkey},bl:: Vector{T4},bu:: Vector{T5}) = putvarboundslice(task,Int32(first),Int32(last),bk,convert(Vector{Float64},bl),convert(Vector{Float64},bu))
...
putvartype{T1}(task:: MSKtask,j:: T1,vartype:: Variabletype) = putvartype(task,Int32(j),vartype)
function putvartype(task_:: MSKtask,j_:: Int32,vartype_:: Variabletype)
  res = disable_sigint() do
    @msk_ccall( "putvartype",Int32,(Ptr{Void},Int32,Int32,),task_.task,j_-1,vartype_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putvartypelist{T1}(task:: MSKtask,subj:: Vector{T1},vartype:: Vector{Variabletype}) = putvartypelist(task,convert(Vector{Int32},subj),vartype)
...
function putxc(task_:: MSKtask,whichsol_:: Soltype)
  ...
  xc_ = __tmp_var_1
  res = disable_sigint() do
    @msk_ccall( "putxc",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,__tmp_var_1)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putxcslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,xc:: Vector{T4}) = putxcslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},xc))
function putxcslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,xc_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putxcslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,xc_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function putxx end
putxx{T2}(task:: MSKtask,whichsol:: Soltype,xx:: Vector{T2}) = putxx(task,whichsol,convert(Vector{Float64},xx))
function putxx(task_:: MSKtask,whichsol_:: Soltype,xx_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putxx",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,xx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putxxslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,xx:: Vector{T4}) = putxxslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},xx))
function putxxslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,xx_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putxxslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,xx_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
"""
function puty end
puty{T2}(task:: MSKtask,whichsol:: Soltype,y:: Vector{T2}) = puty(task,whichsol,convert(Vector{Float64},y))
function puty(task_:: MSKtask,whichsol_:: Soltype,y_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "puty",Int32,(Ptr{Void},Int32,Ptr{Float64},),task_.task,whichsol_.value,y_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putyslice{T2,T3,T4}(task:: MSKtask,whichsol:: Soltype,first:: T2,last:: T3,y:: Vector{T4}) = putyslice(task,whichsol,Int32(first),Int32(last),convert(Vector{Float64},y))
function putyslice(task_:: MSKtask,whichsol_:: Soltype,first_:: Int32,last_:: Int32,y_:: Vector{Float64})
  ...
  res = disable_sigint() do
    @msk_ccall( "putyslice",Int32,(Ptr{Void},Int32,Int32,Int32,Ptr{Float64},),task_.task,whichsol_.value,first_-1,last_-1,y_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function readdataformat(task_:: MSKtask,filename_:: AbstractString,format_:: Dataformat,compress_:: Compresstype)
  res = disable_sigint() do
    @msk_ccall( "readdataformat",Int32,(Ptr{Void},Ptr{UInt8},Int32,Int32,),task_.task,string(filename_),format_.value,compress_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function readsolution(task_:: MSKtask,whichsol_:: Soltype,filename_:: AbstractString)
  res = disable_sigint() do
    @msk_ccall( "readsolution",Int32,(Ptr{Void},Int32,Ptr{UInt8},),task_.task,whichsol_.value,string(filename_))
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function readsummary(task_:: MSKtask,whichstream_:: Streamtype)
  res = disable_sigint() do
    @msk_ccall( "readsummary",Int32,(Ptr{Void},Int32,),task_.task,whichstream_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
removebarvars{T1}(task:: MSKtask,subset:: Vector{T1}) = removebarvars(task,convert(Vector{Int32},subset))
...
removecones{T1}(task:: MSKtask,subset:: Vector{T1}) = removecones(task,convert(Vector{Int32},subset))
...
removecons{T1}(task:: MSKtask,subset:: Vector{T1}) = removecons(task,convert(Vector{Int32},subset))
...
removevars{T1}(task:: MSKtask,subset:: Vector{T1}) = removevars(task,convert(Vector{Int32},subset))
...
function sensitivityreport(task_:: MSKtask,whichstream_:: Streamtype)
  res = disable_sigint() do
    @msk_ccall( "sensitivityreport",Int32,(Ptr{Void},Int32,),task_.task,whichstream_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function solutiondef(task_:: MSKtask,whichsol_:: Soltype)
  isdef_ = Vector{Int32}(1)
  res = disable_sigint() do
    @msk_ccall( "solutiondef",Int32,(Ptr{Void},Int32,Ptr{Int32},),task_.task,whichsol_.value,isdef_)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function solutionsummary(task_:: MSKtask,whichstream_:: Streamtype)
  res = disable_sigint() do
    @msk_ccall( "solutionsummary",Int32,(Ptr{Void},Int32,),task_.task,whichstream_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function updatesolutioninfo(task_:: MSKtask,whichsol_:: Soltype)
  res = disable_sigint() do
    @msk_ccall( "updatesolutioninfo",Int32,(Ptr{Void},Int32,),task_.task,whichsol_.value)
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
function writesolution(task_:: MSKtask,whichsol_:: Soltype,filename_:: AbstractString)
  res = disable_sigint() do
    @msk_ccall( "writesolution",Int32,(Ptr{Void},Int32,Ptr{UInt8},),task_.task,whichsol_.value,string(filename_))
  end
  if res != MSK_RES_OK.value
    msg = getlasterror(task_)
    throw(MosekError(res,msg))
...
putlicensecode{T1}(env:: MSKenv,code:: Vector{T1}) = putlicensecode(env,convert(Vector{Int32},code))
