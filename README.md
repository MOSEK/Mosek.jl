...
- Branch `b1.1-msk9.2` is compatible with MOSEK 9.2. Currently updated only for bugfixes. Since MOSEK 9.2 replaces 9.1, MOSEK.jl 1.1.x will take future releases from this branch.Mosek.jl 
- Since MOSEK 10.0 the branch names of Mosek.jl will follow Mosek, so the branch compatible with MOSEK 10.0 will be called b10.0. Release will be called `MAJORVER.MINORVER.N`, where `N` is incremented independently of MOSEK. This means that since MOSEK 10.0, Mosek.jl vX.Y.Z will require MOSEK version X.Y
